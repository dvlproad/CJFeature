...
@property (nonatomic, copy) void(^buttonHandle)(NSInteger index);
...
///添加多个按钮
- (void)addBottomButtons:(NSArray<UIButton *> *)bottomButtons
  withActionButtonHeight:(CGFloat)actionButtonHeight
          bottomInterval:(CGFloat)bottomInterval
              leftOffset:(CGFloat)leftOffset
             rightOffset:(CGFloat)rightOffset
            buttonHandle:(void(^)(NSInteger index))buttonHandle
{
    NSInteger bottomButtonCount = bottomButtons.count;
    CGFloat bottomButtonsHeight = bottomButtonCount*actionButtonHeight;
    UIView *bottomButtonContaint = [[UIView alloc] init];
    [self addSubview:bottomButtonContaint];
    [bottomButtonContaint mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(-bottomInterval);
        make.height.mas_equalTo(bottomButtonsHeight);
        make.left.mas_equalTo(self).mas_offset(leftOffset);
        make.right.mas_equalTo(self).mas_offset(-rightOffset);
    }];
    _bottomPartHeight = 0 + bottomButtonsHeight + bottomInterval;
    
    _buttonHandle = buttonHandle;
    
    for (UIButton *bottomButton in bottomButtons) {
        [bottomButtonContaint addSubview:bottomButton];
//        [bottomButton addTarget:self action:@selector(bottomButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    [bottomButtons mas_distributeViewsAlongAxis:MASAxisTypeVertical withFixedItemLength:actionButtonHeight leadSpacing:0 tailSpacing:0];
    [bottomButtons mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(20);
        make.centerX.mas_equalTo(bottomButtonContaint);
    }];
}

//- (void)bottomButtonAction:(UIButton *)bottomButton {
//    [self dismissWithDelay:0];
//    
//    NSInteger index = bottomButton;
//    if (self.buttonHandle) {
//        self.buttonHandle(index);
//    }
//}
